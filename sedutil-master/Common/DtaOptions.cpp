...
#define GIT_VERSION "JEFF_VERSION 1.0.0"
//#include "Version.h"
void usage()
{
    ...
    printf("                                print MSID \n");
    printf("\n");
	printf("--writeDataStore <infile> <SIDpassword> <device>\n");
	printf("                                 write a file to the datastore\n");
	printf("--readDataStore <outfile> <SIDpassword> <device>\n");
...
uint8_t DtaOptions(int argc, char * argv[], DTA_OPTIONS * opts)
{
	...
	for (uint8_t i = 1; i < argc; i++) {
		...
		BEGIN_OPTION(readDataStore,3) OPTION_IS(filename) OPTION_IS(password) OPTION_IS(device) END_OPTION
		BEGIN_OPTION(writeDataStore,3) OPTION_IS(filename) OPTION_IS(password) OPTION_IS(device) END_OPTION
		else {
