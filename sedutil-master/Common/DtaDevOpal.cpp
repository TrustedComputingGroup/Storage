...
uint8_t DtaDevOpal::writeDataStore(char* filename, char* password)
{
	...
	uint32_t eofpos;
	uint32_t size = 0;
	uint32_t sizeSize = 4;//4 bytes for the size of the size;

	ifstream inputfile;

	printf("Writing %s to DataStore\n", filename);

	(MAX_BUFFER_LENGTH > tperMaxPacket) ? blockSize = tperMaxPacket : blockSize = MAX_BUFFER_LENGTH;
	if (blockSize > (tperMaxToken - 4)) blockSize = tperMaxToken - 4;
	vector <uint8_t> buffer, lengthtoken, sizetoken;
	blockSize -= sizeof(OPALHeader) + 50;  // packet overhead
	buffer.resize(blockSize);
	...
	eofpos = (uint32_t)inputfile.tellg();
	...
	DtaCommand* cmd = new DtaCommand();
	if (NULL == cmd) {
		printf("Unable to create command object ");
		return DTAERROR_OBJECT_CREATE_FAILED;
	}

	session = new DtaSession(this);
	if (NULL == session) {
		printf("Unable to create session object ");
		return DTAERROR_OBJECT_CREATE_FAILED;
	}
	if ((lastRC = session->start(OPAL_UID::OPAL_LOCKINGSP_UID, password, OPAL_UID::OPAL_ADMIN1_UID)) != 0) {
		delete cmd;
	...
	lengthtoken.clear();
	lengthtoken.push_back(0xe2);
	lengthtoken.push_back((uint8_t)((sizeSize >> 16) & 0x000000ff));
	lengthtoken.push_back((uint8_t)((sizeSize >> 8) & 0x000000ff));
	lengthtoken.push_back((uint8_t)(sizeSize & 0x000000ff));

	sizetoken.clear();
	sizetoken.push_back((uint8_t)((size >> 24) & 0x000000ff));
	sizetoken.push_back((uint8_t)((size >> 16) & 0x000000ff));
	sizetoken.push_back((uint8_t)((size >> 8) & 0x000000ff));
	sizetoken.push_back((uint8_t)(size & 0x000000ff));
	
	cmd->reset(OPAL_UID::OPAL_UID_DATASTORE, OPAL_METHOD::SET);
	cmd->addToken(OPAL_TOKEN::STARTLIST);
	cmd->addToken(OPAL_TOKEN::STARTNAME);
	cmd->addToken(OPAL_TOKEN::WHERE);
	cmd->addToken(filepos);
	...
	filepos += 4; //+4 for the size record
	eofpos += 4; //+4 to compensate for size record.
	while (!inputfile.eof()) {
		if (eofpos == filepos) break;
		if ((eofpos - filepos) < blockSize) {
			blockSize = eofpos - filepos; // handle a short last block
		...
		inputfile.read((char*)buffer.data(), blockSize);
		cmd->reset(OPAL_UID::OPAL_UID_DATASTORE, OPAL_METHOD::SET);
		cmd->addToken(OPAL_TOKEN::STARTLIST);
		cmd->addToken(OPAL_TOKEN::STARTNAME);
		cmd->addToken(OPAL_TOKEN::WHERE);
	...
	delete session;
	inputfile.close();
	printf("%d bytes written to DataStore\n",size);
	LOG(D1) << "Exiting DtaDevOpal::testDataStore()";
	return 0;
}
uint8_t DtaDevOpal::readDataStore(char* outputfile, char* password)
{
	...
	uint8_t lastRC;
	uint32_t size = 0;
	uint8_t* contentBytes = 0;
	uint32_t blockSize = 1792;
	uint32_t contentSize = blockSize;
	uint32_t readBytes = 0;
	uint32_t firstrow = 0;
	uint32_t secondrow = 0;
	//uint32_t nextreadsize = 0;
	uint8_t sizeBytes[4] = { 0 };
	ofstream outstream;
	
	

	printf("Reading file from DataStore\n");
	...
	if ((lastRC = session->start(OPAL_UID::OPAL_LOCKINGSP_UID,
		password, OPAL_UID::OPAL_ADMIN1_UID)) != 0) {
	...
	table.push_back(OPAL_SHORT_ATOM::BYTESTRING8);
	for (int i = 0; i < 8; i++) {
		table.push_back(OPALUID[OPAL_UID::OPAL_UID_DATASTORE][i]);
	}


	if ((lastRC = getTableDataStore(table, 0x00, 0x03)) != 0) {
		printf("unable to get datastore table\n");
		delete session;
		return lastRC;
	}
	...
	response.getBytes(1,sizeBytes);

	size = uint32_t((unsigned char)(sizeBytes[0]) << 24 |
		(unsigned char)(sizeBytes[1]) << 16 |
		(unsigned char)(sizeBytes[2]) << 8 |
		(unsigned char)(sizeBytes[3]));
	
	//printf("size is %d\n", size);
	contentBytes = new uint8_t[blockSize];
	if (NULL == contentBytes)
	{
		printf("failed to instantiate datastore content array\n");
		delete session;
		return DTAERROR_OBJECT_CREATE_FAILED;
	}
	...
	firstrow = 4; //after the size bytes
	


	outstream.open(outputfile, ios::out | ios::binary);

	//if the blockSize is greater than the size of the file just grab the whole file size in one go.
	if (size < blockSize)
	{
		secondrow = size + 3; //+4 for size bytes -1 for 0 index
		blockSize = size;
	}
	else
	{
		secondrow = blockSize + 3;
	}
	...
	while (readBytes < size)
	{
		...
		//printf("reading firstrow %d secondrow %d\n", firstrow, secondrow);

		if ((lastRC = getTableDataStore(table, firstrow, secondrow)) != 0) {
			printf("unable to get datastore table\n");
			delete session;
			delete[] contentBytes;
			outstream.close();
		...
		memset(contentBytes, 0, contentSize); 
		response.getBytes(1, contentBytes);
		

		outstream.write((const char*)contentBytes, (blockSize)); 
		
		readBytes += blockSize;

		if ((size - readBytes) < blockSize)
		{
			blockSize = size - readBytes; // handle a short last block
			//printf("blocksize is now %d", blockSize);
		}
	...
	printf("Read %d bytes to %s from DataStore",readBytes, outputfile);
	//printf("\n");
	outstream.close();
	delete[] contentBytes;
	delete session;
...
uint8_t DtaDevOpal::getTableDataStore(vector<uint8_t> table, uint32_t startrow,
	uint32_t endrow)
{
	LOG(D1) << "Entering DtaDevOpal::getTable";
	uint8_t lastRC;
	...
	get->reset(OPAL_UID::OPAL_UID_DATASTORE, OPAL_METHOD::GET);

	get->addToken(OPAL_TOKEN::STARTLIST);
	get->addToken(OPAL_TOKEN::STARTLIST);
	get->addToken(OPAL_TOKEN::STARTNAME);
	get->addToken(OPAL_TOKEN::STARTROW);
	get->addToken((uint64_t)startrow);
	get->addToken(OPAL_TOKEN::ENDNAME);
	get->addToken(OPAL_TOKEN::STARTNAME);
	get->addToken(OPAL_TOKEN::ENDROW);
	get->addToken((uint64_t)endrow);
	get->addToken(OPAL_TOKEN::ENDNAME);
	get->addToken(OPAL_TOKEN::ENDLIST);
	get->addToken(OPAL_TOKEN::ENDLIST);
